...
@PASS VARCHAR (20)
AS
SELECT USER_WEB,'true' FROM TC_CLIENT WHERE USER_WEB=@USER AND PWD_WEB=@PASS
GO 
...
@USER VARCHAR (20)
AS
SELECT ID_TBCNTCINFO FROM TC_CLIENT WHERE USER_WEB=@USER
GO 
...
@FECHA DATE
AS
INSERT INTO TB_LOGWEBSYS (TC_CLIENT_ID_TBCNTCINFO,DATE_OPERATION) VALUES (@ID_USER,@FECHA);
GO 
...
@USER VARCHAR (20)
AS
SELECT WEB_LOCKED FROM TC_CLIENT WHERE USER_WEB=@USER
GO 
...
@USER VARCHAR (20)
AS
SELECT WEB_ENABLE FROM TC_CLIENT WHERE USER_WEB=@USER
GO 
...
@USER VARCHAR (20)
AS
UPDATE TC_CLIENT set WEB_LOCKED=1 WHERE USER_WEB=@USER
GO 
