...
--Sentencia para traer zona
CREATE PROCEDURE SP_id_zona
@ZONA VARCHAR (100)
AS
SELECT ID_TBCLZONE FROM TB_CL_ZONE WHERE NAME_ZONE = @ZONA
GO 

--Sentencia para guardar el registro de tareas
CREATE PROCEDURE SP_guardar_tarea
@ID_TBCLIENT INT,
@ID_TBCLZONE INT,
@ID_TBINCODEMTR INT,
@DOC_ENTRY VARCHAR(20),
@NUM_SERIAL VARCHAR(20),
@MARK VARCHAR(20),
@NAME_MODEL VARCHAR(20),
@FECHA DATE
AS
INSERT INTO TB_PRELOAD_MTR (ID_TBCLIENT,ID_TBCLZONE,ID_TBINCODEMTR,DOC_ENTRY,STATE_PRELOAD,NUM_SERIAL,MARK,NAME_MODEL,CREATE_DATE) VALUES (@ID_TBCLIENT,@ID_TBCLZONE,@ID_TBINCODEMTR,@DOC_ENTRY,0,@NUM_SERIAL,@MARK,@NAME_MODEL,@FECHA);
GO 

--Sentencia para traer el ID del cliente
CREATE PROCEDURE SP_id_cliente
@USER VARCHAR (20)
AS
SELECT ID_TBCLIENT FROM TB_CLIENT WHERE USER_WEB=@USER
GO 

--Sentencia para traer el ID del codigo de ingreso
CREATE PROCEDURE SP_id_codigos
@CODIGO VARCHAR (50)
AS
SELECT ID_TBINCODEMTR FROM TB_INCODE_MTR WHERE CODE = @CODIGO
GO 

--Sentencia para traer el DOC-ENTRY
CREATE PROCEDURE SP_create_documento
AS
SELECT CONCAT(DOC_ENTRY_TXT,DOC_ENTRY) AS DOCUMENTO FROM TB_CNSC_MTR
GO 

DROP PROCEDURE SP_create_doc_entry

--Sentenciua para incrementar en uno el DOC-ENTRY
CREATE PROCEDURE SP_incrementa_documento
AS
UPDATE TB_CNSC_MTR SET DOC_ENTRY = DOC_ENTRY + 1
GO 

--Sentencia para confirmar registro
CREATE PROCEDURE SP_registro_ok
@CLIENTE INT,
@FECHA DATE
AS
INSERT INTO TB_LOGWEBSYS (ID_TBCLIENT,DATE_OPERATION) VALUES (@CLIENTE,@FECHA);
GO 
